...
const trabalhaPresencial = (pessoaPresencial:Pessoa[]) : Pessoa[] => {
        return pessoaPresencial.filter((funcionario) => {
            return funcionario.presencial === true && funcionario.setor === SETORES.MARKETING
        }
    )
}

console.log(trabalhaPresencial(funcionarios))